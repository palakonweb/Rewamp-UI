...
/**
 * GlowTextChip
 * Dark Glass Text Chip with Blinking Cursor + Animated Rainbow Spotlight.
 *
 * Props:
 * - text (string): Static text to display, or starting word. Defaults to 'Reality'.
 * - words (string[]): Optional list of words to cycle through if typewriter is enabled.
 * - typewriter (boolean): When true, types and deletes words naturally. Defaults to true.
 * - typingSpeed (number): Speed of typing in ms per char. Defaults to 100.
 * - deletingSpeed (number): Speed of deleting in ms per char. Defaults to 50.
 * - pauseDelay (number): Delay between completed word and deletion in ms. Defaults to 2000.
 * - className (string): Optional custom classes for the outer chip container.
 */
export default function GlowTextChip({
  ...
  // Word cycling state for typewriter mode
  ...
  return (
    ...
    >
      {/* Outer ambient chromatic glow reacting to the edge rainbow */}
      <div
      ...
      {/* 1. Pill-ish rounded rectangle chip */}
      <div
      ...
      >
        {/* Large Text inside + Blinking Cursor */}
        <div className="relative z-10 flex items-center">
          ...
          {/* 2. Blinking cursor: Classic hard blink, not a fade */}
          <motion.span
        ...
        {/* Soft wide flowing rainbow gradient */}
        <motion.div
        ...
        {/* Crisp 2.5px solid rainbow gradient bar right at the very edge */}
        <div
          ...
          style={{
            boxShadow: '0 0 10px rgba(255, 122, 24, 0.8), 0 0 16px rgba(255, 45, 149, 0.5)',
          }}
        />
      </div>
    </motion.div>
