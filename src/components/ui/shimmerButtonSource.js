export const shimmerButtonPrompt = `Create a sleek obsidian shimmer button: a tactile dark charcoal pill button ("View Designs →") with a subtle translucent rim border and inner top specular edge. A smooth diagonal specular light reflection periodically sweeps across the pill face from left to right like light catching on a polished convex stone, accelerating on hover with an arrow micro-nudge and spring press physics.`;

export const shimmerButtonCode = `import React, { useState } from 'react';
...
export const ShimmerButton: React.FC<ShimmerButtonProps> = ({
  ...
  return (
    <motion.button
      ...
      className={\`group relative inline-flex items-center justify-center gap-2.5 px-7 py-3 rounded-full border text-[15px] font-medium select-none overflow-hidden cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-[#EC5E27] \${currentVariant.button} \${className}\`}
    >
      ...
      {/* Primary Periodic Shimmer Sweep */}
      <motion.div
        key={\`loop-\${clickCount}\`}
      ...
      <span className={\`relative z-10 tracking-tight \${currentVariant.textColor}\`}>
        {children || text}
      </span>
