...
export default function WireframeRingOrb({
  ...
  useEffect(() => {
    ...
    // Tilt the orb group ~35 degrees like in the video
    rootGroup.rotation.z = THREE.MathUtils.degToRad(32);
    rootGroup.rotation.x = THREE.MathUtils.degToRad(20);
    scene.add(rootGroup);

    // Create concentric cross-section contour rings forming the 3D sphere
    ...
    for (let i = 0; i < ringCount; i++) {
      // Height along z axis from -0.82 to +0.82
      ...
      // Gradient alpha for rings based on distance from equator
      const normalizedDist = Math.abs(tNorm);
      const ringAlpha = THREE.MathUtils.lerp(0.95, 0.55, normalizedDist);
    ...
    const animate = () => {
      ...
      // Dynamic rippling breathing along the rings matching the video
