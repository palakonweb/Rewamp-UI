...
import { pixelSnowPrompt } from './pixelSnowSource';

const promptContent = pixelSnowPrompt;

const jsxSource = `import { useCallback, useEffect, useMemo, useRef } from 'react';
import {
  Color,
  Mesh,
  OrthographicCamera,
...
const cssSource = `.pixel-snow-container {
  width: 100%;
  height: 100%;
  position: relative;
  overflow: hidden;
    ...
    return (
        <div className="w-full rounded-2xl bg-white dark:bg-[#111] border border-black/5 dark:border-white/10 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 border-b border-black/5 dark:border-white/10">
                <p className="text-[11px] text-black/40 dark:text-white/40 uppercase tracking-widest font-semibold">{label}</p>
                <motion.button
                    onClick={handleCopy}
            ...
            <pre className="text-[12px] leading-relaxed font-mono text-black/80 dark:text-white/80 p-5 overflow-x-auto max-h-[400px] overflow-y-auto">
                <code>{code}</code>
            </pre>
        </div>
    );
...
export default function PixelSnowBackgroundShowcase() {
    ...
    return (
        <div className="w-full flex flex-col gap-6 max-w-4xl mx-auto">
            {/* PREVIEW SECTION — live PixelSnow (three.js / WebGL) */}
            <div className="relative w-full h-[500px] rounded-[24px] overflow-hidden border border-black/5 dark:border-white/10 bg-[#050505] shadow-2xl flex items-center justify-center p-8">
...
{/* SOURCE CODE — exact react-bits implementation */}
            ...
            <p className="text-[11px] text-black/40 dark:text-white/40 text-center -mt-2">
                Requires the <code className="font-mono">three</code> package. Source: reactbits.dev/backgrounds/pixel-snow (React Bits, MIT).
            </p>
        </div>
    );
}
