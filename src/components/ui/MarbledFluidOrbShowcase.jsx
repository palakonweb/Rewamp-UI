...
import { Copy, Check, Play, Pause, RotateCcw, Maximize2, Minimize2 } from 'lucide-react';
import MarbledFluidOrb from './MarbledFluidOrb';
import { marbledFluidOrbPrompt } from './marbledFluidOrbSource';
...
export default function MarbledFluidOrbShowcase() {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(marbledFluidOrbPrompt);
    setCopied(true);
    ...
    <div className="w-full flex flex-col gap-6 max-w-4xl mx-auto">
      ...
      <div
        ...
        style={{
          ...
          backgroundImage: `
            radial-gradient(circle at 50% 45%, #FFFFFF 0%, #F5F4F0 60%, #E8E6E0 100%)
          `,
        }}
      >
        {/* Top Controls */}
        ...
        {/* ── WHITE AI THINKING PILL: Marbled Glass Orb on LEFT, Shimmery Text on RIGHT ── */}
        <motion.div
          layout
        ...
        >
          {/* LEFT: 3D Iridescent Marbled Silk Glass Orb */}
          <div className="relative w-10 h-10 shrink-0 flex items-center justify-center rounded-full overflow-hidden filter drop-shadow-[0_2px_8px_rgba(255,100,140,0.25)]">
            <MarbledFluidOrb size={40} speed={1.0} />
          </div>

          {/* RIGHT: Shimmery Reasoning Text */}
      ...
      {/* ── Prompt block ── */}
      <div className="w-full rounded-2xl bg-white border border-mist p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-xs">
