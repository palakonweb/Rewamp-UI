...
async function main() {
  ...
  const htmlContent = `
    ...
    <html>
      <body style="margin: 0; background: #fff; display: flex; align-items: center; justify-content: center; height: 100vh;">
        <video id="vid" src="http://localhost:5174/Recording%202026-09-15%20204056.mp4" style="max-width: 90vw; max-height: 90vh;" muted playsinline></video>
      </body>
    </html>
  `;
  ...
  const outDir = path.resolve('public', 'frames_search');
